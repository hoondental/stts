...
_alpha2kor = {'a':'에이', 'b':'비', 'c':'씨', 'd':'디', 'e':'이', 'f':'에프', 'g':'쥐', 'h':'에이치', 'i':'아이', 'j':'제이', 'k':'케이',
              'l':'엘', 'm':'엠', 'n':'엔', 'o':'오', 'p':'피', 'q':'큐', 'r':'알', 's':'에쓰', 't':'티', 'u':'유', 'v':'브이', 'w':'더블유',
              'x':'엨스', 'y':'와이', 'z':'지'}
...
def convert_eng(string, cmu):
    ...
    for eng_word in eng_words:
        word = eng_word.lower()
        if word not in cmu:
            kword = ''.join([_alpha2kor[c] for c in word])
            string = string.replace(eng_word, kword, 1)
            continue
        ...
        string = string.replace(eng_word, ret, 1)
    return string
